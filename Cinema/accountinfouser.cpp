...
AccountinfoUser::AccountinfoUser(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::AccountinfoUser)
{
    ui->setupUi(this);

     //это добавил - удали
/*    // Создание объектов классов Account и AccountinfoUser
   // Account *account = new Account;
   // AccountinfoUser *accountInfoUser = new AccountinfoUser;

    // Создание соединения сигнала и слота
    //connect(account, SIGNAL(textEntered(QString)), accountInfoUser, SLOT(receiveText(QString)));  */
...
void AccountinfoUser::on_pushButton_Edit_clicked()
{
    Account conn;
...
void AccountinfoUser::on_pushButton_Delete_clicked()
{
    Account conn;
    QString Surname;
    Surname=ui->txt_surname->text();
...
/*void AccountinfoUser::receiveText(QString text)       //эту функцию добавил - удали
{
    this->ui->comboBox->currentText();
}*/

//эта кнопка по идее будет не нужна когда решу
void AccountinfoUser::on_pushButton_load_tab_clicked()
{
    Account conn;
    QString Surname;
    Surname=ui->txt_surname->text();
    ...
    //за место верхнего запроса, хочу передавать не все фамилии, а фамилию которая привязана к логину при входе, только как получить строку из другой формы
    //qry->prepare("select Surname from users where username ='"+lineEdit_username+"'");
...
void AccountinfoUser::on_comboBox_currentIndexChanged(const QString &arg1)
{
...
void AccountinfoUser::on_pushButton_2_clicked()
{
    this->close();
